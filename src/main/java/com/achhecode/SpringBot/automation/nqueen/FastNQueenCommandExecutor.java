...
@Slf4j
@Component
public class FastNQueenCommandExecutor implements NQueenCommandExecutor {
    ...
    /**
     * Delay in milliseconds after each N-Queen command, so the target
     * app has time to process it. Override via:
     *   automation.keyboard.nqueen.command-delay-ms=20
     */
    @Value("${automation.keyboard.nqueen.command-delay-ms:20}")
     ...
     *   automation.keyboard.nqueen.post-switch-delay-ms=200
     */
    @Value("${automation.keyboard.nqueen.post-switch-delay-ms:200}")
    ...
    public void initialize() {
        ...
        log.info("CGEventSource initialized for fast N-Queen keyboard automation");
    }

    @Override
    public synchronized void execute(List<NQueenInputCommand> commands, String executionId) {

        if (commands == null || commands.isEmpty()) {
            log.warn("No N-Queen commands to execute. executionId={}", executionId);
            return;
        }
        ...
        try {
            ...
            for (int i = 0; i < commands.size(); i++) {
                NQueenInputCommand command = commands.get(i);
                if (command == null) {
                    continue;
                }
            ...
            log.info(
                "N-Queen commands executed. executionId={}, count={}, elapsedMs={}",
                executionId, commands.size(), elapsedMs
            );

        } catch (Exception e) {
            log.error("N-Queen keyboard automation failed. executionId={}", executionId, e);
    ...
    private void sleep(long millis) {
        try {
            ...
            throw new NQueenCommandExecutionException(
                "N-Queen keyboard automation interrupted during inter-command delay",
                "n/a",
                e
            );
        }
